...
class DefaultServiceDeploymentsServiceSpec extends WordSpec with Matchers with MockitoSugar with ScalaFutures {
  ...
  "Get all" should {
    ...
    "give all deployments for a given service in production" in {
      Mockito
        ...
        .thenReturn(Future.successful(List(
          ...
          EnvironmentalDeployment(
            "production",
            "some-other-ServiceName",
            "1.0",
            now.minusDays(2),
            Seq(Deployer("abc.xyz", now)))
        )))
      ...
      result("some-other-ServiceName") shouldBe Seq(
        ServiceDeployment("1.0", now.minusDays(2), Seq(Deployer("abc.xyz", now))))
    }
